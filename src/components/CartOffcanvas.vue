...
//     delCart () {
//       // !塞入要刪除的ＩＤ
//       const url = `${import.meta.env.VITE_APP_API}api/${import.meta.env.VITE_APP_PATH}/cart/${this.tempCart.id}`;
...
export default function useCart(product) {
  ...
  const getCart = () => {
    ...
    axios.get(api).then((res) => {
      ...
      carts.value.forEach((item) => {
        sumFinalTotal.value += item.total
        sumFinalQty.value += item.qty
      })
    })
  }

  const openDelCartModel = (item) => {
    tempCart.value = { ...item }
    tempCartTitle.value = { ...item.product }
  ...
  const delCarts = () => {
    if (carts.value.length > 0) {
      ...
      swal.fire('Cart was empty.', '', 'warning')
    }
  }
