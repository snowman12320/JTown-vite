...
export default {
  ...
  methods: {
    ...
    getOrdersAll () {
      const url = `${import.meta.env.VITE_APP_API}api/${import.meta.env.VITE_APP_PATH}/admin/orders?page=${this.tempPage}`;
      this.$http.get(url, this.tempProduct).then((response) => {
    ...
    delAllOrder () {
      // !避免誤刪
      const url = `${import.meta.env.VITE_APP_API}api/${import.meta.env.VITE_APP_PATH}/admin/order/allll`;
      this.isLoading = true;
