...
export default {
  ...
  setup() {
    ...
    const getProducts = () => {
      ...
      axios.get(url).then((response) => {
        ...
        products.value.forEach((product) => {
          if (!uniqueCategories.value.includes(product.category)) {
            uniqueCategories.value.push(product.category)
          }
        })
      })
    }
    ...
    watch(cacheSearch, () => {
      emitter.emit('customEvent_search', cacheSearch.value.title)
      cacheCategory.value = ''
    })
    ...
    watch(cacheCategory, () => {
      cacheSearch.value = ''
      filterCheck.value = ''
      emitter.emit('customEvent_category', cacheCategory.value)
    })

    watch(filterCheck, () => {
      emitter.emit('customEvent_Check', filterCheck.value)
    })
